...
function App() {
  ...
  const currentBaseLabel = BASE_MODELS.find((m) => m.code === baseModel)?.label || "All Models";
  const currentCategoryLabel = CATEGORIES.find((c) => c.code === category)?.label || "All Categories";

  const compactMode = true;
  const hasAnyBlocks = Boolean(blockData?.blocks?.length);
  const isFallbackBlocks = Boolean(blockData?.fallback);
  const fallbackReason = blockData?.fallback_reason || "Fallback generated by backend";
  const dirtyByIndex = useMemo(() => {
    const out = {};
    if (!blockData?.blocks?.length) return out;
    blockData.blocks.forEach((b, idx) => {
      ...
      if (Math.abs(current - original) > 0.0001) out[b.block_index] = true;
    });
    return out;
  }, [blockData, originalBlockWeights]);

  const isDirty = Object.keys(dirtyByIndex).length > 0;
  const effectiveShowSliders = false;

  const blockStats = useMemo(() => {
    const weights = (blockData?.blocks || []).map((b) => clampBlockWeight(Number(b.weight) || 0));
    if (!weights.length) return { min: 0, max: 0, mean: 0, variance: 0 };
  ...
  const visibleBlockWeightsCsv = useMemo(() => {
    if (!blockData?.blocks?.length) return "";
    return blockData.blocks.map((b) => toOneDecimalWeight(Number(b.weight) || 0).toFixed(1)).join(",");
  }, [blockData]);
  ...
  async function runSearch(page = 0) {
    ...
    try {
      ...
      if (baseModel !== "ALL") params.set("base_model_code", baseModel);
      if (category !== "ALL") params.set("category_code", category);
      if (search.trim()) params.set("q", search.trim());
      if (onlyBlocks) params.set("only_with_blocks", "1");
      params.set("limit", String(PAGE_SIZE));
      params.set("offset", String(page * PAGE_SIZE));

      const res = await fetch(`${API_BASE}/lora/search?${params.toString()}`);
      if (!res.ok) throw new Error(`Search failed with status ${res.status}`);
      const data = await res.json();
      const items = sortLoras(data.results || [], sortMode);
      setResults(items);
      setTotalResults(data.total || 0);
      setCurrentPage(page);
      if (!selectedStableId && items.length > 0) {
        // Keep details empty until user clicks; avoids surprise heavy block fetches.
      }
    } catch (err) {
      setErrorMsg(err.message || "Failed to load LoRAs");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function loadProfiles(stableId) {
    if (!stableId) return;
    try {
      ...
      // Non-fatal; profiles panel can be empty.
    } finally {
